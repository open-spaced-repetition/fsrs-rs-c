...
int main() {
    ...
    FSRS* fsrs = fsrs_new(params, sizeof(params) / sizeof(params[0]));

    FSRSReview reviews[] = {{1, 1}, {3, 2}, {3, 3}, {3, 5}, {3, 8}, {3, 13}};
    FsrsReviews review_history = {reviews, sizeof(reviews) / sizeof(reviews[0])};
    FSRSItem* item = fsrs_item_new(&review_history);

    NextStates* next_states = fsrs_next_states(fsrs, NULL, 0.9, 21);

    ItemState again = fsrs_next_states_again(next_states);
    ItemState hard = fsrs_next_states_hard(next_states);
    ItemState good = fsrs_next_states_good(next_states);
    ItemState easy = fsrs_next_states_easy(next_states);
